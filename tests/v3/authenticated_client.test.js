...
if (apiKey) {
  ...
  describe("Test Authenticated EtsyClientV3", function() {
    ...
    if (testPlan.shopSections) {
      it("should get shop sections", async function() {
        ...
        var shopSections = await client.getShopSections()
    ...
    if (testPlan.shopActiveListings) {
      it("should get shop active listings", async function() {
        ...
        logger.info(" - [findAllActiveListingsByShop] get shop active listings ", {'shopId':client.shopId, 'limit':productsLimit });
        var listOptions = {language:'fr', translate_keywords:true, limit: productsLimit, offset:0};
        var activeListings = await client.findAllActiveListingsByShop(listOptions)
    ...
    if(testPlan.shopListingVariationImages) {
      it("should get shop listing variation images", async function() {
        ...
        logger.info(" - [getListingVariationImages] get first product related images ", {listingId, 'limit':imagesLimit});
        var getListingVariationImages = await client.getListingVariationImages(listingId, {limit:imagesLimit})
    ...
    if(testPlan.shopListingImages) {
      it("should get shop listing images", async function() {
        ...
        logger.info(" - [getListingImages] get first product related images ", {listingId, 'limit':imagesLimit});
        var getListingImages = await client.getListingImages(listingId, {limit:imagesLimit})
    ...
    if(testPlan.shopListingProperties) {
      it("should get shop listing properties", async function() {
        ...
        var getListingProperty = await client.getListingProperty(listingId, {'shopId':client.shopId})
                                             .catch(err =>console.log("getListingImages err", err));

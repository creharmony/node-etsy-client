...
import {isSet} from "./commonsUtils.js";

const DEFAULT_DRY_MODE = false;// DEBUG // if true, then print get onto console instead of calling etsy
const SILENT_API_KEY_FORMAT_WARNING = process.env.ETSY_SILENT_API_KEY_WARNING === "true" || process.env.ETSY_SILENT_API_KEY_WARNING === "1";
const API_KEY_FORMAT_WARNED = { warned: false }; // singleton to warn only once
...
class EtsyClientV3 {
    ...
    _assumeShopId() {
        if (!isSet(this.shopId)) {
            throw "shopId is not defined";
        }
    }

    _assumeOAuth2() {
        if (!isSet(this.accessToken)) {
            throw "accessToken is not defined";
        }
    }

    _assumeApiUrl() {
        if (!isSet(this.apiUrl)) {
            throw "apiUrl is required. ie. set ETSY_API_ENDPOINT environment variable.";
        }
    }

    _assumeApiKey() {
        if (!isSet(this.apiKey)) {
            throw "apiKey is required. ie. set ETSY_API_KEY environment variable.";
        }
        this._checkApiKeyFormat();
    }

    _checkApiKeyFormat() {
        if (!this.apiKey || this.apiKey.includes(':')) {
            return; // Format is correct (keystring:secret) or apiKey is not set
        }
        // Format is incorrect (missing shared secret)
        if (!SILENT_API_KEY_FORMAT_WARNING && !API_KEY_FORMAT_WARNED.warned) {
            API_KEY_FORMAT_WARNED.warned = true;
            console.warn(
                '⚠️  ETSY API KEY FORMAT WARNING\n' +
                'Starting January 18, 2026, Etsy requires all API requests to include a shared secret.\n' +
                'Your x-api-key header must use the format: keystring:secret\n' +
                ...
                'To silence this warning, set ETSY_SILENT_API_KEY_WARNING=true'
            );
        }
    }

    _assumeField(fieldName, fieldValue) {
        if (!fieldValue) {
